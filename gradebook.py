...
roster = pd.read_csv(DATA_FOLDER/'roster.csv.txt',
    converters={"NetID": str.lower, "Email Address": str.lower},
    usecols=["Section", "Email Address", "NetID"],
    index_col="NetID",)

hw_exam_grades = pd.read_csv(
    DATA_FOLDER / "hw_exam_grades.csv.txt",
...
for file_path in DATA_FOLDER.glob("quiz_*_grades.csv.txt"): #what is the meaning of *
...
quiz_grades.index=quiz_grades.index.str.lower()
#print(quiz_grades.head(3))
...
while(n<n_exam):
    n=n+1
    n=str(n)
    colName="Exam"+n+"Score"
    final_data[colName]=(final_data["Exam"+" "+n]/final_data["Exam"+" "+n+" "+"-"+" "+"Max Points"])
    n=int(n)
#print(final_data.head(3))
homework_scores = final_data.filter(regex="^Homework\s[0-9]+?$", axis=1)
homework_max_points=final_data.filter(regex="^Homework\s[0-9]+\s.",axis=1)
#print(homework_scores.head(3))
#print(homework_max_points.head(3))
#i>=1
#i=str(i)
#final_data=final_data.loc[:,[True if re.search("^Homework\s[0-9]+?$",i)else False for i in final_data.columns]]
#final_data=final_data.loc[:final_data.str.contains('(^Homework\s[0-9]+?$)',regex=True)]
#print(final_data.head(5))

sum_of_hw_score=homework_scores.sum(axis=1)
sum_of_hw_max=homework_max_points.sum(axis=1)
final_data['Total Homework']=sum_of_hw_score/sum_of_hw_max
...
quiz_scores=final_data.filter(regex="^Quiz\s.$",axis=1)
...
weightings=pd.Series({"Exam1Score":0.05,"Exam2Score":0.1,"Exam3Score": 0.15,"Quiz Score":0.30,"Homework Score":0.4})
final_data["Final Score"]=(final_data[weightings.index]*weightings).sum(axis=1)
percent=final_data['Final Score']*100
final_data["Ceiling Score"]=np.ceil(percent)
...
normal_dict=scipy.stats.norm.pdf(x,loc=final_mean,scale=final_std)
plt.plot(x,normal_dict,label="Normal Distribution",linewidth=4)
plt.legend()
plt.show()
